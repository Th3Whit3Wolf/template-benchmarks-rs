    ...
    #[cfg(feature = "make_md")]
    {
        ...
        let msg = if let Ok(manifest) = cargo_toml_data {
            ...
            for num in 0..top_100_template_engines_crates.len() {
                if !NOT_TEMPLATE_LIB.contains(&top_100_template_engines_crates[num].name.as_str()) && deps
                    .remove_entry(&top_100_template_engines_crates[num].name)
                    .is_some()
                {
                    m.push_str(&crate_into_row(Some(num + 1), &top_100_template_engines_crates[num]))
                }
            }
            for (name, _) in deps {
            ...
            let (_, rustc_date) = get_version_and_date().unwrap();
            let rustc_date=  rustc_date.unwrap();
            let date_as_str = rustc_date.split('-').collect::<Vec<&str>>();
            let dt = chrono::NaiveDate::from_ymd(date_as_str[0].parse::<i32>().unwrap(), date_as_str[1].parse::<u32>().unwrap(), date_as_str[2].parse::<u32>().unwrap());
            m.push_str(&format!(
                "| unranked | {} | {} | {} | {} |\n",
...
fn crate_into_row(rank: Option<usize>, crate_info: &Crate) -> String {
    ...
    format!(
        ...
        crate_info.recent_downloads.unwrap_or(0),
